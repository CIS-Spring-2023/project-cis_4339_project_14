...
// GET zips and count of zips from DB
router.get('/zips', async(req, res, next) => {
    var temp;
    const uniqueZips = [];
    const numberOfZips = [];
    try {
        ...
        return res.json({ "zips": uniqueZips, "counts": numberOfZips });
    } catch (error) {
        console.log(error);
        return next(error);
    }
...
async function getCount(zip) {

    const count = await clients.countDocuments({ 'address.zip': zip });
